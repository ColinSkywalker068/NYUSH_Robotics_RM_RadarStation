...
#### Calibration Guide
(Colin 20251230 translated)

1. Before each match starts, the radar must be calibrated. Use the `calibration.py` script—edit the parameters first, then run it.
2. Select the calibration mode (`'test'`: test mode, `'hik'`: Hikvision camera, `'video'`: USB camera (VideoCapture)), and choose your team side (in `test` mode, it can only be the blue side). See lines 360 and 362 in `calibration.py`.
3. Run `calibration.py`. After adjusting the camera view to a suitable angle, click **“开始标定”**.
4. In order, click the four corresponding pairs of points (8 points total, **white**) on the **地面** in both the camera view and the map view, then click **切换高度**.
5. In order, click the four corresponding pairs of points (8 points total, **green**) on the **R型高地** in both the camera view and the map view, then click **切换高度**.
6. In order, click the four corresponding pairs of points (8 points total, **blue**) on the **环型高地** in both the camera view and the map view, then click **保存计算**.

![Calibration Demo](images/calibration.JPG)
