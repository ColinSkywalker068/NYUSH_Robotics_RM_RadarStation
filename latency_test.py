...
camera_mode = input("camera mode (test/hik/video): ").strip().lower()
if camera_mode not in {"test", "hik", "video"}:
    raise ValueError("camera mode must be one of: test / hik / video")

# ---- Video source configs ----
VIDEO_SOURCE_MODE = "device"      # always defined
VIDEO_DEVICE_INDEX = 1            # default
VIDEO_FILE_PATH = None

if camera_mode == "video":
    VIDEO_SOURCE_MODE = (input("video source mode (device/file) [device]: ").strip().lower() or "device")
    if VIDEO_SOURCE_MODE == "file":
        VIDEO_FILE_PATH = str(Path(input("test file path: ").strip()).expanduser())
    else:
        s = input("video device index [1]: ").strip()
...
import serial
from hik_camera import set_Value
...
frame_lock = Lock()
camera_image = None
camera_ts = None  # perf_counter timestamp when camera_image was last updated

# ---- Latency test configs ----
LATENCY_PRINT_EVERY = 60  # frames
LATENCY_WINDOW = 300
LATENCY_LOG_PATH = "latency_report.txt"  # appended each report; set to None to disable
FREQ_LOG_PATH = "tracking_freq_report.txt"  # appended each report; set to None to disable
# ---- Tracking frequency configs ----
TRACK_WINDOW_SEC = 5.0  # sliding window in seconds for Hz calculation
    ...
    while True:
        ...
        if ret == 0:
            ...
            with frame_lock:
                ...
                camera_ts = time.perf_counter()
        else:
            print("no data[0x%x]" % ret)


def _video_capture_get_device():
    """Old behavior: capture from a USB webcam index."""
    global camera_image, camera_ts
    cam = cv2.VideoCapture(VIDEO_DEVICE_INDEX, cv2.CAP_DSHOW)  # Windows: CAP_DSHOW helps stability
    ...
    while True:
        ...
        if not ret:
            ...
            continue
        with frame_lock:
            camera_image = img  # assignment is fine; copy happens in main thread
            camera_ts = time.perf_counter()
        time.sleep(0.001)


def _video_capture_get_file():
    """New behavior: capture from a local video file, loop when it ends."""
    global camera_image, camera_ts
    ...
    delay = 1.0 / fps if fps and fps > 0 else 1.0 / 30.0  # fallback 30fps

    while True:
        ret, img = cam.read()
        if not ret:
            # End of file or read error -> loop from start
...
def video_capture_get():
    """Wrapper: choose old/new capture based on VIDEO_SOURCE_MODE."""
    if VIDEO_SOURCE_MODE.lower() == "file":
        _video_capture_get_file()
    else:
        _video_capture_get_device()
...
latency_buf = deque(maxlen=LATENCY_WINDOW)
track_history = {}

# 获取相机图像的画幅，限制点不超限
with frame_lock:
    img0 = None if camera_image is None else camera_image.copy()
img_y = img0.shape[0]
img_x = img0.shape[1]
print(img0.shape)
...
while True:
    ...
    for detection in result0:
        ...
        if cls == 'car':
            ...
            #"cropped" altered under guidance
    ...
    if all_filter_data != {}:
        for name, xyxy in all_filter_data.items():
            if xyxy is not None:
                ...
                if name not in track_history:
                    track_history[name] = deque()
                track_history[name].append(now_t)
                if name[0] == "R":
                    color_m = (0, 0, 255)
                else:
                    color_m = (255, 0, 0)
    ...
    if img0_ts is not None:
        latency = te - img0_ts
        latency_buf.append(latency)
        if len(latency_buf) % LATENCY_PRINT_EVERY == 0:
            vals = list(latency_buf)
            vals.sort()
            ...
            rates = {}
            total_updates = 0
            for name, ts_deque in track_history.items():
                while ts_deque and now_t - ts_deque[0] > TRACK_WINDOW_SEC:
                    ts_deque.popleft()
                if ts_deque:
                    hz = len(ts_deque) / TRACK_WINDOW_SEC
                    rates[name] = hz
                    total_updates += len(ts_deque)
            overall_hz = total_updates / TRACK_WINDOW_SEC if track_history else 0.0
            if rates:
                rate_str = ",".join(f"{k}:{v:.2f}Hz" for k, v in sorted(rates.items()))
            else:
                rate_str = "none"
            report = (
                f"latency avg={avg * 1000:.1f}ms p95={p95 * 1000:.1f}ms n={len(vals)} "
                f"targets={targets_in_frame} track_hz={overall_hz:.2f} [{rate_str}]"
            )
            print(report)
            if LATENCY_LOG_PATH:
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                with open(LATENCY_LOG_PATH, "a") as f:
                    f.write(f"{timestamp} {report}\n")
            if FREQ_LOG_PATH:
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                freq_report = f"track_hz={overall_hz:.2f} [{rate_str}]"
                with open(FREQ_LOG_PATH, "a") as f:
                    f.write(f"{timestamp} {freq_report}\n")
    # 绘制UI
