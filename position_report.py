...
# ---- Position report configs ----
POSITION_REPORT_PATH = "tracked_positions.txt"  # appended each report; set to None to disable
POSITION_REPORT_INTERVAL = 2.0  # seconds
...
while True:
    ...
    if all_filter_data != {}:
        for name, xyxy in all_filter_data.items():
            if xyxy is not None:
                ...
                positions[name] = {
                    "map_x": float(filtered_xyz[0]),
                    "map_y": float(filtered_xyz[1]),
                    "ref_x": float(ser_x),
                    "ref_y": float(ser_y),
                }
                # 缩放坐标到地图图像
    ...
    now_wall = time.time()
    if POSITION_REPORT_PATH and now_wall - last_position_report >= POSITION_REPORT_INTERVAL:
        payload = {
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "positions": positions,
        }
        with open(POSITION_REPORT_PATH, "a") as f:
            f.write(json.dumps(payload, ensure_ascii=True) + "\n")
        last_position_report = now_wall
    # 绘制UI
